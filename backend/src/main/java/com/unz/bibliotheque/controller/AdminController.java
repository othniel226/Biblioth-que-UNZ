...
import com.unz.bibliotheque.pattern.factory.UtilisateurFactory;
...
public class AdminController {
    ...
    public String creerUtilisateur(
        ...
        try {
            ...
            Utilisateur user = UtilisateurFactory.creerUtilisateur(roleEnum, nom, prenom, email);
            user.setMotDePasse(passwordEncoder.encode(motDePasse));

            // Champs spécifiques
            if (roleEnum == Role.ETUDIANT && user instanceof Etudiant e) {
                e.setMatricule(matricule != null ? matricule : "MAT-" + System.currentTimeMillis());
                e.setFiliere(filiere != null ? filiere : "Informatique");
                e.setNiveau(niveau != null ? niveau : "L3");
            } else if (roleEnum == Role.BIBLIOTHECAIRE && user instanceof Bibliothecaire b) {
                b.setBadgeNumero(badgeNumero != null ? badgeNumero : "BIB-" + System.currentTimeMillis());
                b.setService(service != null ? service : "Section générale");
            }

            utilisateurRepo.save(user);
            ra.addFlashAttribute("success", "Compte créé avec succès pour " + prenom + " " + nom);
            return "redirect:/admin/utilisateurs";

        } catch (Exception ex) {
            ra.addFlashAttribute("error", "Erreur lors de la création : " + ex.getMessage());
            return "redirect:/admin/utilisateurs/creer";
        }
    }
    ...
    public String activerUtilisateur(@PathVariable Long id, RedirectAttributes ra) {
        ...
        user.setActif(true);
        utilisateurRepo.save(user);
        ra.addFlashAttribute("success", "Compte activé avec succès.");
        return "redirect:/admin/utilisateurs";
    }
    ...
    public String desactiverUtilisateur(@PathVariable Long id, RedirectAttributes ra) {
        ...
        if (user.getEmail().equals(auth.getName())) {
            ra.addFlashAttribute("error", "Vous ne pouvez pas désactiver votre propre compte !");
            return "redirect:/admin/utilisateurs";
        }
        user.setActif(false);
        utilisateurRepo.save(user);
        ra.addFlashAttribute("success", "Compte désactivé avec succès.");
        return "redirect:/admin/utilisateurs";
    }
    ...
    public String rapports(Model model) {
        ...
        model.addAttribute("nbEmpruntsEnCours", empruntRepo.countByStatut(
            com.unz.bibliotheque.model.enums.StatutEmprunt.EN_COURS));
        model.addAttribute("nbEmpruntsEnRetard", empruntRepo.countByStatut(
            com.unz.bibliotheque.model.enums.StatutEmprunt.EN_RETARD));
        model.addAttribute("nbOuvrages", ouvrageRepo.count());
        model.addAttribute("nbEtudiants", utilisateurRepo.countByRole(Role.ETUDIANT));
    ...
    public String configuration(Model model) {
        model.addAttribute("configurations", configurationRepo.findAll());
